...
# 사용법 체크
if [ "$#" -lt 4 ]; then
    echo "Usage: $0 <agent> <domains> <seeds> <snapshot_ts> [additional hydra overrides]"
    echo "Example: $0 sbrl 'walker quadruped' '1 2 3' 2000000 agent.becl_period=30000 agent.becl_duration=0"
    exit 1
fi
...
for DOMAIN in "${DOMAINS[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
        for TASK in ${TASKS[$DOMAIN]}; do
            ...
            # 실행 명령어
            CMD="HYDRA_FULL_ERROR=1 python $EXPERIMENT_DIR/finetunev2.py domain=$DOMAIN task=$TASK obs_type=states agent=$AGENT reward_free=false pretrain_seed=$SEED snapshot_ts=$SNAPSHOT_TS $EXTRA_ARGS"
